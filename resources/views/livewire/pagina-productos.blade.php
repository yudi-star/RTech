                ...
                @foreach($categorias as $categoria)
                 <li class="mb-4" wire:key="{{$categoria->id}}">
                   <label for="{{$categoria->slug}}" class="flex items-center dark:text-gray-400 ">
                     <input type="checkbox" wire:model.live="selected_categorias" id="{{$categoria->slug}}" value="{{$categoria->id}}"  class="w-4 h-4 mr-2">
                ...
                @foreach($marcas as $marca)
                 <li class="mb-4" wire:key="{{$marca->id}}">
                   <label for="{{$marca->slug}}" class="flex items-center dark:text-gray-300">
                     <input type="checkbox" wire:model.live="selected_marcas" id="{{$marca->slug}}" value="{{$marca->id}}" class="w-4 h-4 mr-2">
              ...
              <ul>
                <li class="mb-4">
                  <label for="destacado" class="flex items-center dark:text-gray-300">
                    <input type="checkbox" id="destacado" wire:model.live="destacado" value="1" class="w-4 h-4 mr-2">
                    <span class="text-lg dark:text-gray-400">Destacados</span>
                  </label>
                </li>
                <li class="mb-4">
                  <label for="en_venta" class="flex items-center dark:text-gray-300">
                    <input type="checkbox" wire:model.live="en_venta" id="en_venta" value="1" class="w-4 h-4 mr-2">
                 ...
                 <div class="border border-gray-300 dark:border-gray-700"style="height: auto;">
                   <div class="relative bg-white h-3/4">
                     <a href="/products/{{$producto->slug }}" class="">
                       <img src="{{url('storage', $producto->imagenes[0])}}" alt="{{$producto->nombre}}" style="height: auto;width: 100%; object-fit: cover;" class="object-cover w-full h-56 mx-auto ">
                     </a>
                   </div>
                   <div class="p-3 ">
                     ...
                     <p class="text-lg ">
                       <span class="text-green-600 dark:text-green-600">{{Number::currency($producto->precio,'PEN')}}</span>
                     </p>
                   </div>
